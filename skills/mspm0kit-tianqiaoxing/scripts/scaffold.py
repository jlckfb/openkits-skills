...
def main(
    ...
    for item in source_dir.rglob("*"):
        ...
        if item.suffix == ".syscfg":
            ...
            if "Generated by scaffold" not in content and sdk_example != "blink":
                header = (
            ...
            # Auto-fill direction="OUTPUT" for GPIO output pins missing it
            # (SDK gpio_toggle_output template omits direction/initialValue)
            lines = content.split('\n')
            patched = []
            for i, line in enumerate(lines):
                patched.append(line)
                # After a pin assignment line, check if direction is missing
                if '.associatedPins[' in line and '.$name' in line:
                    # Look ahead: if no direction in next 6 lines for this pin, add it
                    prefix = line.split('.$name')[0]  # e.g. "GPIO1.associatedPins[0]"
                    has_direction = any(
                        f"{prefix}.direction" in lines[j]
                        for j in range(i + 1, min(i + 7, len(lines)))
                    )
                    if not has_direction:
                        indent = line[:len(line) - len(line.lstrip())]
                        patched.append(f'{indent}{prefix}.direction        = "OUTPUT";')
                        patched.append(f'{indent}{prefix}.initialValue     = "SET";')
            content = '\n'.join(patched)
